...
from GreysonBot.modules.helper_funcs.handlers import CMD_STARTERS, SpamChecker
...
if is_module_loaded(FILENAME):
    ...
    from GreysonBot.modules.sql import disable_sql as sql
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "GreysonBot.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "GreysonBot.modules." + args[0].rsplit(".", 1)[0]
